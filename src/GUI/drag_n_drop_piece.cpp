...
int main(int argc, char* args[]) {
    ...
    while (!quit) {
            ...
            if (event.type == SDL_QUIT) {
                ...
                if (!isPieceSelected) {
                    ...
                    if (x < BOARD_SIZE * SQUARE_SIZE && y < BOARD_SIZE * SQUARE_SIZE) {
                        ...
                        fromSquare = getBoardCoordinates(x, y);
                        selectedPieceRect = {x - SQUARE_SIZE / 2, y - SQUARE_SIZE / 2, SQUARE_SIZE, SQUARE_SIZE};
                    }
                }
                ...
                if (isPieceSelected) {
                    ...
                    // Check if a valid square is released
                    if (x < BOARD_SIZE * SQUARE_SIZE && y < BOARD_SIZE * SQUARE_SIZE) {
                        std::string toSquare = getBoardCoordinates(x, y);
        ...
        if (isPieceSelected) {
            ...
            SDL_RenderFillRect(renderer, &selectedPieceRect);
        }
